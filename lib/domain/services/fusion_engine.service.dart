...
final class FusionEngine {
  ...
  Future<String?> findFusionCandidate(ScreenshotEntity newScreenshot) async {
    ...
    final List<ScreenshotEntity> recent = await _screenshotRepository
        .getRecent();
    final Set<String> newTokens = _tokenize(newScreenshot.ocrText);
    if (newTokens.isEmpty) return null;

    for (final ScreenshotEntity candidate in recent) {
      if (candidate.uuid == newScreenshot.uuid) continue;
      if (candidate.cardUuid == null) continue;

      final Set<String> candTokens = _tokenize(candidate.ocrText);
      if (candTokens.isEmpty) continue;

      final double jaccard = _jaccard(newTokens, candTokens);
      if (jaccard >= _kJaccardThreshold) {
        _log.info(
          'Fusion candidate found (jaccard=${jaccard.toStringAsFixed(2)}) →'
          ' ${candidate.cardUuid}',
        );
        // Vérifie que la Card existe toujours.
        final CardEntity? existing = await _cardRepository.getByUuid(
          candidate.cardUuid!,
        );
