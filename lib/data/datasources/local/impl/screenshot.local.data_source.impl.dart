...
final class ScreenshotLocalDataSourceImpl implements ScreenshotLocalDataSource {
  ScreenshotLocalDataSourceImpl({required ObjectBoxStore store})
    : _store = store;

  final ObjectBoxStore _store;

  Box<ScreenshotLocalModel> get _box =>
      _store.store.box<ScreenshotLocalModel>();

  @override
  Future<ScreenshotLocalModel?> getByUuid(String uuid) async {
    final Query<ScreenshotLocalModel> q = _box
        .query(ScreenshotLocalModel_.uuid.equals(uuid))
        .build();
    try {
      return q.findFirst();
    } finally {
  ...
  Future<ScreenshotLocalModel?> getBySha256(String sha256) async {
    final Query<ScreenshotLocalModel> q = _box
        .query(ScreenshotLocalModel_.sha256.equals(sha256))
        .build();
    try {
      return q.findFirst();
    } finally {
  ...
  Future<ScreenshotLocalModel> upsert(ScreenshotLocalModel screenshot) async {
    final ScreenshotLocalModel? existing = await getByUuid(screenshot.uuid);
  ...
  Future<List<ScreenshotLocalModel>> getByCardUuid(String cardUuid) async {
    final Query<ScreenshotLocalModel> q = _box
        .query(ScreenshotLocalModel_.cardUuid.equals(cardUuid))
        .build();
    try {
      return q.find();
    } finally {
  ...
  Future<List<ScreenshotLocalModel>> getRecent(Duration within) async {
    ...
    final Query<ScreenshotLocalModel> q = _box
        .query(
          ScreenshotLocalModel_.capturedAt.greaterThan(
  ...
  Future<void> linkToCard(String screenshotUuid, String cardUuid) async {
    final ScreenshotLocalModel? s = await getByUuid(screenshotUuid);
    if (s == null) return;
    s.cardUuid = cardUuid;
    _box.put(s);
  }
  ...
  Future<void> delete(String uuid) async {
    final ScreenshotLocalModel? s = await getByUuid(uuid);
