...
final class NotificationRecordLocalDataSourceImpl
  ...
  NotificationRecordLocalDataSourceImpl({required ObjectBoxStore store})
    : _store = store;
  ...
  @override
  Future<NotificationRecordLocalModel> upsert(
    NotificationRecordLocalModel record,
  ) async {
    final NotificationRecordLocalModel? existing = await getByUuid(record.uuid);
  ...
  Future<NotificationRecordLocalModel?> getByUuid(String uuid) async {
    final Query<NotificationRecordLocalModel> q = _box
        .query(NotificationRecordLocalModel_.uuid.equals(uuid))
        .build();
    try {
      return q.findFirst();
    } finally {
  ...
  @override
  Future<List<NotificationRecordLocalModel>> byTypeWithin(
    String type,
    Duration within,
  ) async {
    final DateTime threshold = DateTime.now().subtract(within);
    final Query<NotificationRecordLocalModel> q = _box
        .query(
          NotificationRecordLocalModel_.type
              .equals(type)
              .and(
  ...
  Future<void> purgeOlderThan(Duration age) async {
    final DateTime threshold = DateTime.now().subtract(age);
    final Query<NotificationRecordLocalModel> q = _box
        .query(
        ...
        .build();
    try {
      final List<NotificationRecordLocalModel> old = q.find();
      _box.removeMany(
        old.map((NotificationRecordLocalModel e) => e.id).toList(),
      );
    } finally {
      q.close();
    }
